...
import { useVegetables, useFarmerBills, useFarmers } from '@/lib/storage';
import { useSettings } from '@/lib/useSettings';
import { BillItem, Sack, Vegetable } from '@/lib/types';
import {
  ArrowLeft, Trash2, IndianRupee, Save, AlertCircle, CornerDownLeft, Package, X, Percent
} from 'lucide-react';
...
export default function EditFarmerBillPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();
  const { vegetables, loaded: vegsLoaded } = useVegetables();
  const { farmerBills, updateFarmerBill, loaded: billsLoaded } = useFarmerBills();
  ...
  useEffect(() => {
    if (bill && !initialized) {
      setDate(bill.date);
      setCommissionRate(String(bill.commissionRate));
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const oldBalance = bill!.newBalance;
      await updateFarmerBill(id, {
      ...
      router.push(`/farmer-bills/${id}`);
    } finally { setSaving(false); }
  };

  if (!mounted || !billsLoaded || !vegsLoaded || !settingsLoaded || !farmersLoaded) {
    return (
  ...
  if (!bill) {
    return (
      <div className="p-6 lg:p-8 text-center">
        ...
        <Link href="/farmer-bills" className="text-yellow-600 hover:underline">Back to Farmer Bills</Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-8 space-y-5 max-w-3xl">
      <div className="flex items-center gap-3">
        <Link href={`/farmer-bills/${id}`} className="text-gray-400 hover:text-gray-600 p-1"><ArrowLeft className="w-5 h-5" /></Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Bill #{bill.billNumber}</h1>
          <p className="text-gray-500 text-sm">{bill.farmerName}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        ...
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)}
          className="border border-gray-400 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors" />
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input ref={rateRef} type="number" min="0" step="0.5" value={entryRate}
              ...
              placeholder="Rate / kg"
              className="w-full border border-gray-400 rounded-lg pl-7 pr-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors" />
          </div>
          ...
          <div className="relative">
            <input
              ...
              className={clsx('w-full border rounded-lg px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors',
            ...
            {entryVeg && (
              <div className="mt-2">
                ...
                <input
                  ...
                  className="w-full border border-gray-400 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors"
                />
              </div>
            )}
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input ref={sackWeightRef} type="number" min="0" step="0.1" value={entrySackWeight}
                ...
                className={clsx('w-full border rounded-lg pl-9 pr-2 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors',
            ...
            <button type="button" onClick={commitItem}
              ...
              className="flex items-center gap-1.5 bg-yellow-600 hover:bg-yellow-700 disabled:bg-gray-200 disabled:text-gray-400 text-white px-4 py-3 rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
      ...
      {/* Deductions & Payment */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
        <h2 className="font-semibold text-gray-900">கழிவுகள் & கட்டணம் (Deductions & Payment)</h2>
          ...
          <div>
            ...
            <div className="relative w-36">
              <input type="number" min="0" max="100" step="0.5" value={commissionRate}
                ...
                className="w-full border border-gray-400 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors" />
          ...
          <div>
            ...
            <div className="relative w-36">
              ...
              <input type="number" min="0" step="1" value={coolie} onChange={(e) => setCoolie(e.target.value)}
                placeholder="0"
                className="w-full border border-gray-400 rounded-lg pl-9 pr-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors" />
            </div>
          </div>
          <div>
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
          ...
          {commissionRateVal > 0 && <div className="flex justify-between text-red-600"><span>கமிஷன் {commissionRateVal}%</span><span className="font-semibold">−₹{fmtINR(commission, 2)}</span></div>}
          {coolieVal > 0 && <div className="flex justify-between text-red-600"><span>கூலி</span><span className="font-semibold">−₹{fmtINR(coolieVal, 2)}</span></div>}
          {vadakaiVal > 0 && <div className="flex justify-between text-red-600"><span>வாடகை</span><span className="font-semibold">−₹{fmtINR(vadakaiVal, 2)}</span></div>}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input type="number" min="0" step="0.50" value={amountPaid}
              ...
              placeholder="0.00"
              className="w-full border border-gray-400 rounded-lg pl-9 pr-3 py-3 text-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors" />
          </div>
          {totalToPay > 0 && (
            <div className="flex gap-2 mt-2 flex-wrap">
              <button type="button" onClick={() => setAmountPaid(totalToPay.toFixed(2))} className="text-xs border border-yellow-200 text-yellow-700 px-3 py-1 rounded-full hover:bg-yellow-50 transition-colors">Pay Full ₹{fmtINR(totalToPay, 0)}</button>
              <button type="button" onClick={() => setAmountPaid(netAmount.toFixed(2))} className="text-xs border border-gray-200 text-gray-600 px-3 py-1 rounded-full hover:bg-gray-50 transition-colors">Net only ₹{fmtINR(netAmount, 0)}</button>
      ...
      <div className="flex gap-3 pb-4">
        <Link href={`/farmer-bills/${id}`} className="flex-1 sm:flex-none border border-gray-200 text-gray-700 px-6 py-3 rounded-xl font-medium hover:bg-gray-50 transition-colors text-center">Cancel</Link>
        <button type="button" onClick={handleSave} disabled={saving}
          className="flex-1 flex items-center justify-center gap-2 bg-yellow-600 hover:bg-yellow-700 text-white px-6 py-3 rounded-xl font-medium transition-colors shadow-sm disabled:opacity-60">
